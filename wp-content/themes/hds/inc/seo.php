...
/**
 * Approved SEO metadata for category landing pages (SEO-001 §3.2, §3.3).
 *
 * @param string $slug Page slug.
 * @return array{title:string, description:string} Title and description, or empty array.
 */
function hds_get_category_landing_seo( string $slug ): array {
	$data = [
		'glas-en-gevel' => [
...
/**
 * Explicit document title for category landing pages (SEO-001 §3.2).
 *
 * @param string $title The document title generated by WordPress.
 * @return string The document title, possibly overridden.
 */
function hds_category_landing_document_title( string $title ): string {
	if ( is_page() ) {
		$seo = hds_get_category_landing_seo( get_post_field( 'post_name', get_queried_object_id() ) );
		if ( ! empty( $seo['title'] ) ) {
			return $seo['title'];
		}
	}
...
add_filter( 'pre_get_document_title', 'hds_category_landing_document_title' );
...
function hds_add_default_meta_description(): void {
	...
	if ( is_page() ) {
		$landing_seo = hds_get_category_landing_seo( get_post_field( 'post_name', get_queried_object_id() ) );
		if ( ! empty( $landing_seo['description'] ) ) {
			printf(
				'<meta name="description" content="%s">' . "\n",
				esc_attr( $landing_seo['description'] )
			);
			return;
		}
	}
